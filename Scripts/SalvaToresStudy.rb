class SalvaToresStudy < Aura
  
  def self.get_base_stats
    return {
      :name => "Salva Tore's Study",
      :tier  => 1
    }
  end
  ...
  def self.get_description
    return [
      "Artifacts that grant life",
      "or archive grant even more.",
      "",
      "Gain an Salva Tore with a",
      "Giltwork Legacy."
    ]
  end
  ...
  def extra_init
    ...
    # Gain an Salva Tore with a Giltwork Legacy
    give_walker_with(Salva, Artifact_GiltworkLegacy)
  ...
  def apply(impact)
    ...
    buffers = [:MAX_LIFE, :MAX_LIFE_MULTI, :ARCHIVE, :ARCHIVE_MULTI]
