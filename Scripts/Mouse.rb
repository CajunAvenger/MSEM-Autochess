...
module Mouse
    ...
    def self.hold(this)
      @holding = this
      if @holding.is_a?(Unit)
        $player.storefront.update_sell_sprite
        @holding.sprite.lift(10)
        @holding.sprite.on_mouse = true
      elsif @holding.is_a?(Sprite_Artifact)
        $player.storefront.update_sell_sprite
        @holding.lift(10)
        @holding.on_mouse = true
      elsif @holding.is_a?(Sprite_Scheduler)
        @holding.lift(10)
        @holding.on_mouse = true
      end
    end
    ...
    # drop what we're holding, update the sprite if needed
    def self.drop(send_home=false)
      temp = @holding
      @holding = nil
      $player.storefront.update_sell_sprite
      temp.on_mouse = false if temp.is_a?(Sprite_Scheduler)
      if temp.is_a?(Unit) || temp.is_a?(Artifact)
        # drop sprite
        unless temp.sprite.disposed?
          temp.sprite.on_mouse = false
          temp.sprite.lift(-10)
        end
      elsif temp.is_a?(Sprite)
        unless temp.disposed?
          temp.on_mouse = false
          temp.lift(-10)
        end
      end
      if send_home
        # Something was dropped somewhere unexpected, send it back home
        if temp.is_a?(Unit)
          # try to put it back
          if temp.current_hex.battler
            if temp.current_hex.is_bench
              temp.move_to_bench
            else
              temp.owner.move_to_free_hex(temp)
            end
          else
            temp.current_hex.battler = temp
            temp.update_hex(temp.current_hex, true)
          end
        elsif temp.is_a?(Sprite_Artifact)
          $artifact_tray.give(temp)
        elsif temp.is_a?(Sprite)
          # putting a shop item back
          # fade back the shop sprite
          # dispose the sprite on the mouse
          # drop the sprite
          prev_info = temp.holder
    ...
    # set an item as being hovered over
    def self.hover(item)
      @hovering = item
    end
    
    # check if we're hovering over an item
    def self.hovering_over?(item)
      return @hovering == item
    end
    
    # check if we're hovering over an item and not holding anything
    def self.hovering_on?(item)
      return false if @holding
      return @hovering == item
    end
    
    # what section of the screen is the mouse in?
    def self.section
      screen_x, screen_y = Mouse.pos
      return :artifact_scene if $artifact_scene
      return :aura_scene if $aura_scene
      if $infobox.visible
        return :infobox if screen_x.between?($infobox.x, $infobox.x+$infobox.bitmap.width) && screen_y.between?($infobox.y, $infobox.y+$infobox.bitmap.height)
      end
      if $my_auras
        return :auras if screen_x.between?($my_auras.sprite.x, $my_auras.sprite.x+$my_auras.sprite.bitmap.width) && screen_y.between?($my_auras.sprite.y, $my_auras.sprite.y+$my_auras.sprite.bitmap.height)
      end
      return :synergy if screen_x.between?(0, 128) && screen_y < 540
      return :timer if screen_x.between?(128, 703) && screen_y < 33
      return :artifact if screen_x.between?(90, 736) && screen_y > 607
      return :store if screen_x > 736 #&& screen_y < 540
      return :hexes
    end
    #--------------------------------------------------------------------------  
    # * Mouse Grid Position  
    ...
    #     id : 0:Left, 1:Right, 2:Center  
    #--------------------------------------------------------------------------  
    def self.release?(id = 0)
      if @triggers[id][0] >= 0
        return 0
      else
        return -@triggers[id][0]
      end
    end
    #--------------------------------------------------------------------------  
    # * Screen to Client  
